...
export function AdminTabBody({ tab }: { tab: AdminTab }) {
  ...
  async function handleMaterialUploadSubmit(e: React.FormEvent) {
    ...
    setActionLoading("material-upload");
    try {
  ...
  async function handleDeleteCourse(courseId: string) {
    if (!token || isActionBusy(actionLoading) || !window.confirm("Are you sure you want to delete this course? All uploaded materials for this course will be deleted.")) return;
  ...
  async function handleDeleteMaterial(materialId: string) {
    if (!token || isActionBusy(actionLoading) || !window.confirm("Permanently delete this material and its file from storage?")) return;
  ...
  async function handleAddStoryBook(e: React.FormEvent) {
    ...
    try {
      ...
      setBookForm({ title: "", author: "", category: "Playgroup", fileType: "PDF", audience: "BOTH" });
      setBookFile(null);
  ...
  async function handleDeleteStoryBook(bookId: string) {
    if (!token || isActionBusy(actionLoading) || !window.confirm("Are you sure you want to delete this story book?")) return;
  ...
  async function handleDeleteGallery(galleryId: string) {
    if (!token || isActionBusy(actionLoading) || !window.confirm("Are you sure you want to delete this gallery item?")) return;
  ...
  async function handleDeleteTestimonial(testimonialId: string) {
    if (!token || isActionBusy(actionLoading) || !window.confirm("Are you sure you want to delete this testimonial?")) return;
  ...
  return (
    <PortalPageShell>
          ...
          {loading && activeTab !== "users" && activeTab !== "teachers" && activeTab !== "payments" && activeTab !== "planner" ? (
            ...
            <div className={activeTab === "overview" ? portalDashboardBodyClass : "space-y-5 animate-fade-in flex-1 flex flex-col min-h-0 overflow-y-auto"}>
              {/* ────────────────── OVERVIEW TAB ────────────────── */}
              {activeTab === "overview" && (
                <div className={portalDashboardBodyClass}>
                  ...
                  <div className={portalDashboardLowerGridClass}>
                    ...
                    <div className="lg:col-span-2 flex flex-col min-h-0 h-full">
                      ...
                      <div id="recent-transactions" className="bg-white rounded-2xl p-5 border border-slate-200 flex-1 flex flex-col min-h-0 h-full">
                    ...
                    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
                      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-200 animate-scale-up text-slate-800 relative">
                        ...
                        <form onSubmit={handleAssignTask} noValidate className="space-y-4 text-xs">
                          ...
                          <button type="submit" disabled={actionLoading === "task-assign"} className="w-full py-3 rounded-xl bg-[#8AC926] text-white font-sans font-bold text-xs tracking-wider uppercase hover:bg-[#78B020] transition shadow-md shadow-[#8AC926]/10 disabled:opacity-60 disabled:cursor-not-allowed">
                    ...
                    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
                      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-200 animate-scale-up text-slate-800 relative">
                        ...
                        <form onSubmit={handleAddStoryBook} noValidate className="space-y-4 text-xs">
                          ...
                          <div>
                            <label className="block text-slate-700 font-bold mb-1.5">Class (for student library)</label>
                            <PillSelect
                        ...
                        <>
                          ...
                          {googleReviews.length === 0 ? (
                            ...
                            <div className="grid md:grid-cols-2 gap-4 max-h-[65vh] overflow-y-auto pr-1">
                              ...
                              {googleReviews.filter((r) => !r.content || r.content === "—").length > 0 && (
                                <p className="md:col-span-2 text-2xs text-slate-500 font-medium text-center py-2">
                                  {googleReviews.filter((r) => !r.content || r.content === "—").length} additional
                                  star-only rating(s) hidden — Google did not include written text.
                                </p>
                              )}
                            </div>
                          )}
                    ...
                    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
                      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-200 animate-scale-up relative">
                        ...
                        <form onSubmit={handleAddTestimonial} noValidate className="space-y-4 text-xs">
                          ...
                          <button type="submit" disabled={actionLoading === "testimonial-save"} className="w-full py-3 rounded-xl bg-[#8AC926] text-white font-sans font-bold text-xs tracking-wider uppercase hover:bg-[#78B020] transition shadow-md shadow-[#8AC926]/10 disabled:opacity-60 disabled:cursor-not-allowed">
